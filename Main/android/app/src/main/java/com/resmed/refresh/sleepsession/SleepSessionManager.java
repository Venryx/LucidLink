...
public class SleepSessionManager
  implements EdfLibCallbackHandler, RM20Callbacks
{
  ...
  public void start(long paramLong, int paramInt1, int paramInt2)
  {
    ...
    for (localObject1 = this.unitTestContext;; localObject1 = this.serviceHandler.getContext().getApplicationContext())
    {
      ...
      this.rm20Manager.startupLibrary(paramInt1, paramInt2);
      this.rm20Manager.startRespRateCallbacks(true);
      Object localObject2 = SmartAlarmDataManager.getInstance();
      if (((SmartAlarmDataManager)localObject2).getActiveAlarm()) {
        setRM20AlarmTime(new Date(((SmartAlarmDataManager)localObject2).getAlarmDateTime()), ((SmartAlarmDataManager)localObject2).getWindowValue() * 60);
      }
      String str = RefreshModelController.getInstance().getBoardVersion();
      localObject2 = RefreshModelController.getInstance().getFirmwareVersion();
      RefreshModelController.getInstance().saveRM20LibraryVersion(rm20Version());
      Log.d("com.resmed.refresh.bluetooth", "SleepTrackFragment::startSleepSession boardVersion : " + str + " firmwareVersion:" + (String)localObject2);
      localRstEdfMetaData.addMetaField(RstEdfMetaData.Enum_EDF_Meta.unitVer, str);
      localRstEdfMetaData.addMetaField(RstEdfMetaData.Enum_EDF_Meta.rstEdfLibVer, (String)localObject2);
...
/* Location:              C:\Root\@Objects\Tablet\Resmed\App Inspect\JD Gui\com.resmed.refresh-158.jar!\com\resmed\refresh\sleepsession\SleepSessionManager.class
