        ...
        new GuardedAsyncTask<Void, Void>(getReactApplicationContext()) {
            ...
            protected void doInBackgroundGuarded(Void... params) {
                ...
                try {
                    ...
                    // custom; set volume, if specified
					final AudioManager am = (AudioManager)getReactApplicationContext().getSystemService(Context.AUDIO_SERVICE);
					int amStreamMusicMaxVol = am.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
					final int oldVolume = am.getStreamVolume(AudioManager.STREAM_MUSIC);
					float volume = args.hasKey("volume") ? (float)args.getDouble("volume") : -1;
					if (volume != -1) {
						am.setStreamVolume(AudioManager.STREAM_MUSIC, (int) (volume * amStreamMusicMaxVol), 0);
					}
					...
					// custom; reset volume
					if (volume != -1) {
						final Timer uiModifierTimer = new Timer();
						uiModifierTimer.scheduleAtFixedRate(new TimerTask() {
							@Override
							public void run() {
								if (tts.isSpeaking()) return; // if still speaking, keep waiting
								am.setStreamVolume(AudioManager.STREAM_MUSIC, oldVolume, 0);
								uiModifierTimer.cancel();
							}
						}, 100, 100);
					}
